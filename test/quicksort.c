...
main(int argc, char **argv)
{
	...
	while(1) {
		int opt = getopt(argc, argv, "sn:t:x:");
		if(opt < 0)
			break;
		switch(opt) {
		case 's':
	...
	for(int xi = 0; xi < x; xi++)
	{
		...
		delay = end.tv_sec + end.tv_nsec / 1000000000.L -
			(begin.tv_sec + begin.tv_nsec / 1000000000.L);
		for(int i = 0; i < n - 1; i++) {
			assert(a[i] <= a[i + 1]);
		}
	}
 ...
 usage:
	printf("quicksort [-n size] [-t threads] [-s] [-x times]\n");
	return 1;
}
